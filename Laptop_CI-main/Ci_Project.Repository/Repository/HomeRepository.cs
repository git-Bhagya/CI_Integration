...
namespace Ci_Project.Repository.Repository
{
    public class HomeRepository:IHomeRepository
    {
        ...
        //Volunteer Page
        public GeneralViewModel VolunteerUser(int id,int uid)
        {
            GeneralViewModel gvm = new GeneralViewModel();
        ...
        public GeneralViewModel StoryDetailsUser(string search,int id,long uid,int Mid, int count)
        {
            ...
            var missionId = _db.Missions.Where(x => x.MissionId == Mid).Select(x => x.MissionId).FirstOrDefault();
            var storyid = _db.Stories.Where(x => x.UserId == id && x.MissionId == missionId).Select(x => x.StoryId).FirstOrDefault();
