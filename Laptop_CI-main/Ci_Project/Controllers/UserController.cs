...
namespace Ci_Project.Controllers
{
    public class UserController : Controller
    {
        ...
        //userDetails
        public IActionResult SaveDetails(UserViewModel userView,string [] skills)
            {
            var identity = User.Identity as ClaimsIdentity;
            var uid = identity?.FindFirst(ClaimTypes.Sid)?.Value;

            var email = identity?.FindFirst(ClaimTypes.Email)?.Value;
            _userrepository.SaveUserDetails(userView, skills, Convert.ToInt32(uid));
        ...
        public IActionResult getDetailsforhrs(int id, string email, DateTime date, int hour, int mins, string desc, string descG, DateTime dateG, int action)
        {
            var x = _userrepository.getDetailsAndUpdate(id, email, date, hour, mins, desc, descG, dateG, action);
            return View(x);
        }
    }
}
